...
import type { ProjectResults } from "@/lib/decision-tree";
import { DataExplorer } from "@/components/data-explorer";
import { PuzzlePredictor } from "@/components/puzzle-predictor";
import { TryPuzzle } from "@/components/try-puzzle";
...
function DownloadsSection() {
  const files = [
    ...
    },
    {
      ...
      size: "27.5 MB",
      desc: "Complete Next.js source code for THIS website — includes the analysis script (scripts/run_analysis.py) that re-runs the notebook on the CSV, the trained Decision Tree as JSON, all interactive components, plus README with self-hosting instructions.",
...
function InteractiveSections() {
      ...
      <section className="border-y border-border bg-muted/30">
        <div className="mx-auto max-w-7xl px-6 py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Could not load live data ({error}). The interactive sections below need
            <code className="mx-1 rounded bg-muted px-1.5 py-0.5 font-mono text-xs">/data/results.json</code>
            which is generated by running the notebook analysis.
          </p>
        </div>
      </section>
    );
      ...
      <section className="border-y border-border bg-muted/30">
        <div className="mx-auto max-w-7xl px-6 py-16">
          <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
            ...
            <div>
              <div className="font-medium">Loading interactive sections…</div>
              <div className="text-sm text-muted-foreground">
                Fetching real puzzle data + decision tree from <code className="font-mono text-xs">/data/results.json</code>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <DataExplorer puzzles={results.samplePuzzles} />
      <PuzzlePredictor
        tree={results.decisionTree}
        scaler={results.scaler}
        classNames={results.classNames}
      />
      <TryPuzzle
        puzzles={results.samplePuzzles}
        tree={results.decisionTree}
        scaler={results.scaler}
      />
    </>
